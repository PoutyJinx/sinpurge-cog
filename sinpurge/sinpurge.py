...
class SinPurge(commands.Cog):
    ...
    def can_timeout_member(self, guild: discord.Guild, moderator: discord.Member, target: PurgeTarget) -> Tuple[bool, str]:
        """Check whether SIN Purge can apply the automatic 5-minute timeout."""
        member = target.member
        ...
        if member.top_role >= bot_member.top_role:
            return False, "Timeout skipped because my bot role is not above the target's highest role."

        return True, ""

    async def apply_auto_timeout(self, guild: discord.Guild, moderator: discord.Member, target: PurgeTarget) -> Tuple[bool, str]:
        """Automatically timeout a current member for 5 minutes before purging.

        Banned or left users cannot be timed out because they are no longer guild members,
        but their old messages can still be purged by ID or exact name.
        """
        ...
        member = target.member
        timeout_until = datetime.now(timezone.utc) + timedelta(minutes=AUTO_TIMEOUT_MINUTES)
        audit_reason = f"SIN Purge automatic {AUTO_TIMEOUT_MINUTES}-minute timeout by {moderator} ({moderator.id})"

        try:
            await member.timeout(timeout_until, reason=audit_reason)
            return True, f"Applied automatic `{AUTO_TIMEOUT_MINUTES}` minute timeout."
        except discord.Forbidden:
            return False, "Timeout failed because Discord denied the action. Check bot role position and `Timeout Members` permission."
        except discord.HTTPException:
            return False, "Timeout failed because Discord rejected the request. The purge will still continue."
        except AttributeError:
            return False, "Timeout failed because this discord.py/Red version does not expose `member.timeout`."
    ...
    async def sinpurge_prefix(self, ctx: commands.Context, *, args: str):
        ...
        timeout_applied, timeout_note = await self.apply_auto_timeout(ctx.guild, ctx.author, target)
    ...
    async def sinpurge_slash(
        ...
        timeout_applied, timeout_note = await self.apply_auto_timeout(guild, moderator, target)
